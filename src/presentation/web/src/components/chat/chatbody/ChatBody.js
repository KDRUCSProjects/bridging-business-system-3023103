...
import ServerUrl from '../../../base/chat/api/serverUrl';
...
const ChatBody = ({ match, currentChattingMember, setOnlineUserList }) => {
  ...
  const fetchChatMessage = async () => {
    ...
    if (currentChatId) {
      const url =
        // ApiEndpoints.CHAT_MESSAGE_URL.replace(Constants.CHAT_ID_PLACE_HOLDER, currentChatId) + '?limit=20&offset=0';
        `${ApiEndpoints.CHAT_MESSAGE_URL.replace(Constants.CHAT_ID_PLACE_HOLDER, currentChatId)} ?limit=20&offset=0`;
    ...
    if (chatId === data.roomId) {
      if (data.action === SocketActions.MESSAGE) {
        ...
        setMessages((prevState) => {
          const messagesState = JSON.parse(JSON.stringify(prevState));
  ...
  return (
    <div className="col-12 col-sm-8 col-md-8 col-lg-8 col-xl-10 pl-0 pr-0">
      <div className="py-2 px-4 border-bottom d-none d-lg-block">
        <div className="d-flex align-items-center py-1">
          <div className="position-relative">
            <img src={currentChattingMember?.image} className="rounded-circle mr-1" alt="User" width="40" height="40" />
          </div>
      ...
      <div className="position-relative">
        <div id="chat-message-container" className="chat-messages pl-4 pt-4 pr-4 pb-1 d-flex flex-column-reverse">
          {typing && (
            ...
            <div key={index} className={getChatMessageClassName(message.user)}>
              <div>
                <img
                  ...
                  height="40"
                />
                <div className="text-muted small text-nowrap mt-2">{CommonUtil.getTimeFromDate(message.timestamp)}</div>
              </div>
        ...
        <form onSubmit={messageSubmitHandler}>
          <div className="input-group">
            <input
              ...
              autoComplete="off"
            />
            <button id="chat-message-submit" className="btn btn-outline-warning">
              Send
            </button>
          </div>
        </form>
